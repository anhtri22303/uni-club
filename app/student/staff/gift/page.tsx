...
export default function StaffGiftPage() {
  ...
  // 1. Lấy clubId của staff (từ token hoặc context)
  useEffect(() => {
    const id = getClubIdFromToken()
    console.log("🔍 [Staff Gift] Club ID from token:", id)
    if (id) {
      setClubId(id)
    } else {
      toast({
  ...
  const {
    ...
    queryFn: async () => {
      console.log("🚀 [Staff Gift] Fetching event orders for clubId:", clubId)
      const result = await getAllEventOrdersByClub(clubId!)
      console.log("✅ [Staff Gift] Received orders:", result?.length || 0, "items")
      console.log("📦 [Staff Gift] Orders data:", result)
      return result
    },
    enabled: !!clubId,
    staleTime: 3 * 60 * 1000,
        ...
        <div className="space-y-6">
          <div>
            <div className="flex items-center justify-between">
              <div>
                <h1 className="text-3xl font-bold">Staff Gift Management</h1>
                <p className="text-muted-foreground">
                  Manage event product redemption orders
                </p>
              </div>
              <Button
                size="lg"
    ...
    <div className="grid md:grid-cols-2 gap-4">
      {orders.map((order) => {
        ...
        return (
          <Link 
            key={order.orderId} 
            href={`/student/staff/gift/${order.orderId}`}
            className="group"
          >
