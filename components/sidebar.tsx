...
const navigationConfig = {
  ...
  club_leader: [
    ...
    {
      href: "/club-leader/attendances",
      label: "Attendances",
      icon: CalendarDays,
    },
    {
      href: "/club-leader/feedbacks",
      label: "Feedbacks",
      icon: MessagesSquare,
    },
    {
    ...
    },
    {
      ...
      children: [
        ...
        {
          href: "/club-leader/club-order-list",
          label: "Club orders list",
          icon: Truck,
        },
        {
          href: "/club-leader/event-order-list",
          label: "Event orders list",
          icon: Truck,
        },
      ],
    },
  ...
  uni_staff: [
    ...
    { href: "/uni-staff/disciplines", label: "Disciplines", icon: Scale },
    {
      ...
      children: [
        {
          href: "/uni-staff/clubs-req",
          label: "Club Requests",
          icon: Building,
        },
        {
          href: "/uni-staff/points-req",
          label: "Points Requests",
          icon: HandCoins,
        },
        {
          href: "/uni-staff/events-req",
...
export function Sidebar({ onNavigate, open = true }: SidebarProps) {
  ...
  const [pendingMemberAppsCount, setPendingMemberAppsCount] =
    useState<number>(0);
  ...
  const [pendingLeaveRequestsCount, setPendingLeaveRequestsCount] =
    useState<number>(0);

  // State for Club Leader event counts (from getEventByClubId)
  const [clubLeaderPendingEventsCount, setClubLeaderPendingEventsCount] =
    useState<number>(0); // PENDING_COCLUB + PENDING_UNISTAFF
  const [clubLeaderApprovedEventsCount, setClubLeaderApprovedEventsCount] =
    useState<number>(0); // APPROVED
  const [clubLeaderOngoingEventsCount, setClubLeaderOngoingEventsCount] =
    useState<number>(0); // ONGOING

  // State for Club Leader pending club orders (from getClubRedeemOrders)
  const [pendingClubOrdersCount, setPendingClubOrdersCount] =
    useState<number>(0); // PENDING orders

  // State for Uni Staff pending club applications (from useClubApplications)
  const [
    pendingUniStaffClubApplicationsCount,
    setPendingUniStaffClubApplicationsCount,
  ] = useState<number>(0); // PENDING club applications

  // State for Uni Staff pending event requests (from useEvents)
  const [
    pendingUniStaffEventRequestsCount,
    setPendingUniStaffEventRequestsCount,
  ] = useState<number>(0); // PENDING_COCLUB + PENDING_UNISTAFF events

  // State for Uni Staff pending point requests (from usePointRequests)
  const [
    pendingUniStaffPointRequestsCount,
    setPendingUniStaffPointRequestsCount,
  ] = useState<number>(0); // PENDING point requests
  ...
  const { data: memberAppsData } = useMyMemberApplications(
    auth.role === "student"
  );
  ...
  const { data: uniStaffClubApplicationsData } = useClubApplications(
    auth.role === "uni_staff"
  );
    ...
    if (auth.role === "student") {
      const fetchEventsForStudent = async () => {
        try {
          ...
          const nonExpiredEvents = allEvents.filter(
            (event: Event) => !isEventExpired(event)
          );
  ...
  useEffect(() => {
    if (auth.role === "student") {
      ...
      const pendingClubApps = clubApps.filter(
        (app: any) => app.status === "PENDING"
      );
    ...
    if (auth.role === "club_leader") {
      const fetchClubLeaderEvents = async () => {
        try {
          ...
          // Filter events that are not expired
          const nonExpiredEvents = events.filter(
            (event: Event) => !isEventExpired(event)
          );
          ...
          console.error(
            "Failed to fetch events for club leader sidebar:",
            error
          );
          setClubLeaderPendingEventsCount(0);
          setClubLeaderApprovedEventsCount(0);
  ...
  useEffect(() => {
    if (auth.role === "uni_staff" && uniStaffPointRequestsData) {
      ...
      const pendingPointRequests = pointRequests.filter(
  ...
  let navigation = (navigationConfig[
    auth.role as keyof typeof navigationConfig
        ...
        <div className="flex-1 overflow-auto py-4 min-h-0">
          <nav className="grid gap-1 px-2">
            {navigation.map((item, index) => {
              ...
              const usersCount =
                auth.role === "admin" ? userStatsTotal : users.length;
              ...
              const isClubLeaderEventsItem =
                auth.role === "club_leader" && isEventsItem;
              ...
              return (
                <div key={item.href || item.label}>
                  ...
                  <Button
                    ...
                    onMouseEnter={() =>
                      item.href && handleMouseEnter(item.href)
                    }
                    disabled={isLoading}
                  >
                    {isLoading ? (
                  ...
                  {hasChildren && isDropdownOpen && (
                    <div className="ml-4 mt-1 space-y-1">
                      {item.children?.map((child) => {
                        ...
                        const isChildLoading = child.href
                          ? loadingPath === child.href
                          : false;

                        // Child badges
                        const isChildClubRequests =
                          child.label === "Club Requests";
                        const isChildEventRequests =
                          child.label === "Event Requests";
                        const isChildPointsRequests =
                        ...
                        return (
                          <Button
                            ...
                            onMouseEnter={() =>
                              child.href && handleMouseEnter(child.href)
                            }
